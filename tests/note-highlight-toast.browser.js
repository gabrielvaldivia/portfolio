...
(async () => {
  ...
  const tap = index => paragraphs[index].dispatchEvent(new MouseEvent('click', { ...point(index), bubbles: true }))
  const leave = () => root.dispatchEvent(new PointerEvent('pointerleave', { pointerType: 'mouse' }))
  const close = () => panel()?.closest('[data-sonner-toast]').querySelector('[data-close-button]').click()
  ...
  try {
    ...
    leave()
    await pause(450)
    check('leaving keeps the toast while restoring saved ink', Boolean(panel()) && !root.querySelector('[data-note-highlight-hover]'))
    ...
    await pause(8200)
    check('keyboard focus pauses dismissal', Boolean(panel()))
    button('Next reader').click()
    await pause(100)
    ...
    close()
    await pause(250)
    check('close dismisses the toast', !panel())
    tap(0)
    await pause(150)
    check('tapping opens the same toast', text().includes('Brooklyn'))
    failNext = true
    ...
    tap(0)
    await pause(200)
    check('removal preserves the other reader', text().includes('London') && !button('Remove your highlight'))
