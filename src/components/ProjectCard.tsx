...
export function ProjectCard({ title, slug, subtitle, featuredImage, year, href, icon }: Props) {
  return (
    <Link href={href || `/work/${slug}`} className="group block tablet:p-2">
      <div className="rounded-[20px] tablet:rounded-[30px] desktop:rounded-[40px] overflow-hidden bg-background-alt group-hover:bg-elevated transition-colors flex flex-col">
        {/* Square image area */}
